...
public class Movement2DEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.PropertyField(serializedObject.FindProperty("movingType"), new GUIContent("Moving Type", "Type of movement"));
        ...
        if (type == Movement2D.MovingType.Platformer)
        {
            ...
            if (_dash.boolValue)
            {
                ...
                EditorGUILayout.BeginVertical("box");
                EditorGUILayout.HelpBox("Adjust the size and position of the dash collider as needed to fit your character's sliding behavior. You can modify these values in the Unity editor by adjusting the collider's scale and offset. Make sure to input the updated values here for precise control over sliding maneuvers during a dash.", MessageType.Info);
            ...
            _jumpDebug = EditorGUILayout.Foldout(_jumpDebug, "Advanced Jump Settings");
            if (_jumpDebug)
            {
        ...
        else if (type == Movement2D.MovingType.TopDown)
        {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("movementSpeed"), new GUIContent("Movement Speed", "Max speed of the player"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("speedUpAccelaration"), new GUIContent("Speed Up Acceleration", "Acceleration for speed up"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("speedDownAccelaration"), new GUIContent("Speed Down Acceleration", "Acceleration for speed down"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("stopAccelaration"), new GUIContent("Stop Acceleration", "Acceleration for stopping"));
            ...
            if (_dash.boolValue)
            {
                ...
                EditorGUILayout.PropertyField(serializedObject.FindProperty("cancelDashOnWallHit"), new GUIContent("Cancel Dash On Wall Hit", "Cancel dash on hitting a wall"));

                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashDistance"), new GUIContent("Dash Distance", "Distance of the dash"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashDuration"), new GUIContent("Dash Duration", "Duration of the dash"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashStopEffect"), new GUIContent("Dash Stop Effect", "Effect when dash stops"));

                EditorGUILayout.Space();

                EditorGUILayout.PropertyField(serializedObject.FindProperty("airDash"), new GUIContent("Air Dash", "Allow dash while in the air"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashCancelsGravity"), new GUIContent("Dash Cancels Gravity", "Dash cancels gravity"));

                EditorGUILayout.PropertyField(serializedObject.FindProperty("verticalDash"), new GUIContent("Vertical Dash", "Allow vertical dash"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("horizontalDash"), new GUIContent("Horizontal Dash", "Allow horizontal dash"));

                EditorGUILayout.Space();

                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashColliderScale"), new GUIContent("Dash Collider Scale", "Scale of dash collider"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashColliderOffset"), new GUIContent("Dash Collider Offset", "Offset of dash collider"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashParticle"), new GUIContent("Dash Particle", "Particle effect for dash"));

                EditorGUILayout.Space();

                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashShakeDuration"), new GUIContent("Dash Shake Duration", "Duration of dash shake"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashShakeMagni"), new GUIContent("Dash Shake Magnitude", "Magnitude of dash shake"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("dashShakeFreq"), new GUIContent("Dash Shake Frequency", "Frequency of dash shake"));
            }
        }
